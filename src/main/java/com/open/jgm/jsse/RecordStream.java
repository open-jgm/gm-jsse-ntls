...
public class RecordStream {
    ...
    public static final int MAX_CIPHERTEXT_SIZE = 16384; // 16KB
    ...
    public void write(Record record) throws IOException {
        ...
        if (bytes.length > MAX_CIPHERTEXT_SIZE) {
            throw new IOException("Ciphertext exceeds maximum allowed size");
        }
    ...
    public Record read(boolean encrpted) throws IOException {
        ...
        int type = input.read();
        Record.ContentType contentType = Record.ContentType.getInstance(type);
    ...
    public Record encrypt(Record record) throws IOException {
        ...
        if (encrypted.length > MAX_CIPHERTEXT_SIZE) {
            throw new IOException("Encrypted data exceeds maximum allowed size");
        }
